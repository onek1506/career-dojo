...
export default function Slide02Margins({
  currentStep,
  totalSteps,
  onBack,
  onNext,
    ...
    >
      <div className="flex flex-col gap-5 sm:gap-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <CompanyTable
            label="Echte Firmen · Bruttomarge"
            rows={companyExamples.map((c) => ({ name: c.name, value: c.grossMargin }))}
          />
          <CompanyTable
            label="Echte Firmen · Nettomarge"
            rows={companyExamples.map((c) => ({ name: c.name, value: c.netMargin }))}
          />
        </div>
