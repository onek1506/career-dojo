...
const ANSWER_VALUES = ['5%', '15%', '20%', '85%'] as const;
const CORRECT_VALUE = '15%';
const LETTERS = ['A', 'B', 'C', 'D'] as const;

type State = 'idle' | 'submitted-wrong-1' | 'submitted-wrong-2' | 'submitted-correct';

export default function Slide05QuizNet({
  currentStep,
  totalSteps,
  onBack,
  onNext,
    ...
    >
      <div className="flex flex-col gap-5">
        ...
        <h2 className="font-[family-name:var(--font-is-serif)] text-xl sm:text-3xl font-medium text-is-text-primary leading-tight">
          SoftCo · Umsatz €100M, Jahresüberschuss €15M. Wie hoch ist die Nettomarge?
        </h2>
            ...
            >
              <div className="bg-is-bg-secondary border border-is-bg-border rounded-md p-3">
                <p className="font-[family-name:var(--font-is-mono)] text-xs sm:text-sm text-is-text-secondary leading-relaxed">
                  ...
                  <span className="text-is-text-primary">
                    Jahresüberschuss / Umsatz = 15 / 100 = 15%
                  </span>
                </p>
              </div>
              ...
              {isCorrect && (
                <>
                  <MarcusNote
                    ...
                    body={
                      <>
                        Genau. 15% Nettomarge ist Tech-Niveau (Apple, Microsoft) — bei Supermärkten wären das traumhafte Werte. Die Formel sitzt jetzt.
                      </>
                    }
                  />
                  {solvedOnAttempt === 1 ? (
              ...
              {isWrongFirst && (
                <MarcusNote
                  ...
                  body={
                    <>
                      Bei der Nettomarge teilst du den <strong className="not-italic">Jahresüberschuss</strong> durch den Umsatz — nicht umgekehrt, und nicht die Differenz. Probier&apos;s nochmal.
                    </>
                  }
                />
              )}
              ...
              {isWrongFinal && (
                <>
                  <MarcusNote
                    tone="gentle"
                    body={<>Kein Stress. Beim nächsten Mal sitzt sie. Weiter geht&apos;s.</>}
                  />
