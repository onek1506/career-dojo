...
const TAG_COLORS: Record<ProTip['tag'], { text: string; dot: string }> = {
  LERNTIPP: { text: 'text-is-accent', dot: 'bg-is-accent' },
  MERKE: { text: 'text-is-gold', dot: 'bg-is-gold' },
  'NÄCHSTER SCHRITT': { text: 'text-is-success', dot: 'bg-is-success' },
};

export default function Slide07ProTips({
  currentStep,
  totalSteps,
  onBack,
  onNext,
    ...
    >
      <div className="flex flex-col gap-5 sm:gap-6">
        <h2 className="font-[family-name:var(--font-is-serif)] text-3xl sm:text-4xl font-medium text-is-text-primary leading-tight">
          Drei Dinge, die dir das Lernen leichter machen.
        </h2>

        <div className="overflow-hidden -mx-2" ref={emblaRef}>
          <div className="flex">
            {proTips.map((tip) => {
              const color = TAG_COLORS[tip.tag];
              return (
