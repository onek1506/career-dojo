...
export default function Slide09QuizWalkThrough({
  currentStep,
  totalSteps,
  onBack,
  onNext,
  ...
  const [OPTIONS] = useState(() =>
    shuffle(quiz2.options).map((opt, i) => ({ ...opt, id: LETTERS[i] }))
  );
            ...
            >
              <div className="bg-is-bg-secondary border border-is-bg-border rounded-md p-3">
                <p className="font-[family-name:var(--font-is-mono)] text-xs sm:text-sm text-is-text-secondary leading-relaxed">
                  Erklärung <span className="text-is-text-primary">{quiz2.workingOut}</span>
                </p>
              </div>
              ...
              {isWrongFinal && (
                <>
                  <MarcusNote tone={tone} body="Kein Problem. Schau dir die Erklärung oben an." />
