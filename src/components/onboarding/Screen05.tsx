'use client';

import ChoiceCard from './ChoiceCard';
import OnboardingShell from './OnboardingShell';
import type { ScreenProps } from './types';

const OPTIONS = [
  'Was eine Firma in einem Zeitraum verdient hat',
  'Was eine Firma zu einem Stichtag besitzt und schuldet',
  'Wie viel Cash eine Firma generiert',
  'Habe ich noch nie gehört',
];

export default function Screen05({ state, update, navigateTo }: ScreenProps) {
  const ready = state.quiz1 !== null;

  return (
    <OnboardingShell
      step={4}
      showBack
      onBack={() => navigateTo(4)}
      footer={
        <button
          type="button"
          disabled={!ready}
          onClick={() => navigateTo(6)}
          className={`w-full rounded px-5 py-4 text-base font-semibold transition-colors ${
            ready
              ? 'bg-[var(--accent-streak)] text-black hover:brightness-110'
              : 'cursor-not-allowed bg-[var(--border)] text-[var(--text-muted)]'
          }`}
        >
          Auswerten →
        </button>
      }
    >
      <div>
        <p className="text-xs uppercase tracking-[0.1em] text-[var(--text-secondary)]">Frage 1 von 3</p>
        <div className="mt-2 h-0.5 w-full overflow-hidden rounded bg-[var(--border)]">
          <div className="h-full bg-[var(--accent-streak)]" style={{ width: '33%' }} />
        </div>
      </div>

      <div className="mt-5 flex items-center gap-3 rounded border border-[var(--border)] bg-[var(--bg-card)] px-4 py-3">
        <div className="flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-full bg-[var(--border)] font-[family-name:var(--font-jetbrains)] text-[0.65rem] text-[var(--accent-streak)]">
          MH
        </div>
        <span className="text-sm italic leading-snug text-[var(--text-secondary)]">
          Drei kurze Fragen — damit ich dich nicht mit Dingen langweile, die du schon kennst.
        </span>
      </div>

      <h2 className="mt-7 font-[family-name:var(--font-instrument)] text-[clamp(1.8rem,4vw,2.5rem)] leading-tight tracking-tight text-[var(--text-primary)]">
        Was zeigt eine Bilanz?
      </h2>

      <div className="mt-6 flex flex-col gap-3">
        {OPTIONS.map((label) => (
          <ChoiceCard
            key={label}
            selected={state.quiz1 === label}
            onClick={() => update({ quiz1: label })}
          >
            <div className="text-[0.95rem] text-[var(--text-primary)]">{label}</div>
          </ChoiceCard>
        ))}
      </div>
    </OnboardingShell>
  );
}
